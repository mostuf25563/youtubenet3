...
function getAuthenticSrtTrack(lang) {
  ...
  const srtPath = import_path2.default.join(process.cwd(), "test/fixtures/languages", `${cleanLang}.srt`);
...
async function startServer() {
  ...
  if (process.env.NODE_ENV !== "production") {
    ...
    app.get("/demo", (req, res) => {
      res.sendFile(import_path2.default.join(distPath, "demo", "index.html"));
    });
    app.get("/demo/*", (req, res) => {
      res.sendFile(import_path2.default.join(distPath, "demo", "index.html"));
    });
